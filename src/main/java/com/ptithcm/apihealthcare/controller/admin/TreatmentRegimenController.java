...
public class TreatmentRegimenController {
    ...
    @PostMapping(value = "/create-prescription",
            produces = {MediaType.APPLICATION_JSON_VALUE})
    @ResponseBody
    public ResponseEntity<?> createPrescription(@RequestBody PrescriptionParam prescriptionParam){

        Prescription result = prescriptionService.addPrescription(prescriptionParam);
        if(result == null)  return ResponseEntity.ok(new ObjectResponse("0","Tạo đơn thuốc không thành công",false,null));
        else return ResponseEntity.ok(new ObjectResponse("1","Tạo đơn thuốc thành công",true,result));
    }
}
